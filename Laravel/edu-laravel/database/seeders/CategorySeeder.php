...
{
    // 초기 데이터 생성
    // 1. 시더 클래스를 따로 생성(database\seeders에 생성됨) : php artisan make:seeder CategorySeeder
    // 2. database\seeders\DatabaseSeeder->run() 안에 1에서 생성한 시더 클래스를 콜에 추가
    // 3. 1에서 생성한 시더 클래스에 레코드 추가 코드를 작성
    //4-1. 개별시더 실행 : php artisan db:seed --class = 클래스명(= CategorySeeder)
    ...
    public function run()
    {
        ...
        DB::table('categories')->insert([
            ...
            ,['no' => '8', 'name' => '카테고리8']
            ,['no' => '9', 'name' => '카테고리9']
        ]); // db connection -> insert됨 (라라벨에서 지원하는 쿼리빌더 사용해서 만드는 방법임)
